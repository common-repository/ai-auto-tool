...
class aiautotool_AutoGenerateUsername extends rendersetting{
    ...
    public function register_settings() {
        ...
        add_settings_section('auto_generate_username_section', esc_html__('Create user random','ai-auto-tool'), array($this, 'section_callback'), 'auto-generate-username-settings');
        add_settings_field('user_count', esc_html__('Number user:','ai-auto-tool'), array($this, 'user_count_callback'), 'auto-generate-username-settings', 'auto_generate_username_section');
    }

    public function section_callback() {
        echo esc_html__('Create user random','ai-auto-tool');
    }

    public function user_count_callback() {
        $count = get_option($this->user_count_option, 1);
        echo '<input type="number" name="' . esc_attr($this->user_count_option) . '" value="' . esc_attr($count) . '" />';
    }
    ...
    public function render_setting() {
        ?>
         ...
         <div id="tool-create_user-random"  class="tab-content" style="display:none;">
            <h1><i class="fa-solid fa-shield-halved"></i> <?php esc_html_e('Auto create author', 'ai-auto-tool'); ?></h1>
        <div class="wrap">
           <h3><i class="fa-regular fa-pen-to-square"></i> <?php esc_html_e('Input number author ', 'ai-auto-tool') ?></h3>
            <form method="post" action="">
                <p class="ft-note"><i class="fa-solid fa-lightbulb"></i><?php esc_html_e('Quantity.', 'ai-auto-tool'); ?></p>
                <input type="number" name="quantity" id="quantity" required min="1" value="1">
                
                <br><br>

                <p class="ft-note"><i class="fa-solid fa-lightbulb"></i><?php esc_html_e('Password.', 'ai-auto-tool'); ?></p>
    ...
    public function render_tab_setting() {
        if ($this->active=='true') {
         echo '<button href="#tool-create_user-random" class="nav-tab sotab"><i class="fa-solid fa-shield-halved"></i> '.esc_html__('Create profile user author','ai-auto-tool').'</button>';
        }
    }

    public function render_feature() {

       $autoToolBox = new AutoToolBox(esc_html__('Tool Create Random user','ai-auto-tool'), esc_html__('auto Create Random user Author','ai-auto-tool'), "#", $this->active_option_name, $this->active,plugins_url('../images/logo.svg', __FILE__));
